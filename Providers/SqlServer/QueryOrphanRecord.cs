...
public class QueryOrphanRecord : BaseTurboQuery, IQueryOrphanRecord 
{
    /// <summary>
    ...
    public async Task<T> GetOrphanRecordAsync<T>(string query, Action<SqlCommand> setParameters, Func<SqlDataReader, T> mapFunction)
    {
    ...
    public T GetOrphanRecord<T>(string query, Action<SqlCommand> setParameters, Func<SqlDataReader, T> mapFunction)
    {
        using (SqlConnection conn = new SqlConnection(ConnectionString))
        {
            using (SqlCommand cmd = new SqlCommand(query, conn))
